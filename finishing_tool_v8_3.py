# ====================== #
# Finishing Tool v8 beta #
# Nat Cagle 2021-11-15   #
# ====================== #
import arcpy
...
fc_fields_og = { 'AeronauticCrv' : ['F_CODE','FCSUBTYPE','AXS','PCF','SBB','TXP','ZI005_FNA','ZI006_MEM','ZI019_ASP','ZI019_ASP2','ZI019_ASP3','ZI019_ASU','ZI019_ASU2','ZI019_ASU3','ZI019_ASX','ZI019_SFS','ZI026_CTUU','ZI001_SRT','Shape','Version'],
              ...
              'PortHarbourSrf' : ['F_CODE','FCSUBTYPE','FFN','FFN2','FFN3','HGT','LMC','MCC','MCC2','MCC3','PCF','PWC','TID','WLE','ZI005_FNA','ZI006_MEM','ZI025_WLE','ZI026_CTUU','ZI001_SRT','Shape','Version'],
...
#tracking = arcpy.GetParameter(1)
repair = arcpy.GetParameter(1)
fcode = arcpy.GetParameter(2)
defaults = arcpy.GetParameter(3)
metrics = arcpy.GetParameter(4)
ufi = arcpy.GetParameter(5)
hydro = arcpy.GetParameter(6)
hlarge = arcpy.GetParameter(7)
trans = arcpy.GetParameter(8)
tlarge = arcpy.GetParameter(9)
util = arcpy.GetParameter(10)
ularge = arcpy.GetParameter(11)
dups = arcpy.GetParameter(12)
explode = arcpy.GetParameter(13)
error_count = 0
...
try:
	...
	write("MetadataSrf and ResourceSrf removed")
	featureclass.sort()
except:
	write("MetadataSrf and ResourceSrf not present")
	featureclass.sort()
	pass
...
write("Disabling Editor Tracking for ~ {0} ~\n".format(gdb_name))
...
if fcode:
	...
	for fc in featureclass:
		try:
			try:
				write("Updating " + str(fc) + " Feature F_Codes")
...
if hydro:
	...
	if not hlarge:
		try:
			feat_count = arcpy.GetCount_management("hc_scale").getOutput(0)
			write("Integrating {0} {1} features".format(feat_count, fc))
...
if trans:
	...
	fc = 'TransportationGroundCrv'
	write("- - - - - - - - - - - - - - - - - - - - - - ")
	write("Making {0} feature layers".format(fc))
	arcpy.MakeFeatureLayer_management(fc, "tg")
	arcpy.SelectLayerByAttribute_management("tg", "NEW_SELECTION", "zi026_ctuu >= 50000")
	arcpy.MakeFeatureLayer_management("tg", "tg_scale")
	write("Repairing {0} lines before Integration".format(fc))
	arcpy.RepairGeometry_management("tg_scale", "DELETE_NULL")
	if not tlarge:
		try:
			feat_count = arcpy.GetCount_management("tg_scale").getOutput(0)
			write("Integrating {0} {1} features".format(feat_count, fc))
			arcpy.Integrate_management('tg_scale', "0.03 Meters")
		except arcpy.ExecuteError:
			# if the code failed for the current fc, check the error
			error_count += 1
			write("\n***Failed to run {0}.***\n".format(tool_name))
		...
		try:
			...
			with arcpy.da.SearchCursor("tgrid", ['OID@']) as scursor:
				for row in scursor:
					...
					arcpy.SelectLayerByLocation_management("tg_scale", "INTERSECT", "tgrid","","NEW_SELECTION")
					feat_count = arcpy.GetCount_management("tg_scale").getOutput(0)
					write("Integrating {0} {1} features in partition {2}".format(feat_count, fc, row[0]))
					arcpy.Integrate_management("tg_scale", "0.03 Meters")
					# Potentially integrate points like culverts as well
			write("Freeing partition memory")
			arcpy.Delete_management("in_memory")
	...
	arcpy.RepairGeometry_management("tg_scale", "DELETE_NULL")
	write("Clearing process cache")
	arcpy.Delete_management("tg")
	arcpy.Delete_management("tg_scale")
	write("- - - - - - - - - - - - - - - - - - - - - - ")

if util:
	tool_name = 'Integrate and Repair Utilities'
	...
	if not ularge:
		try:
			feat_count1 = arcpy.GetCount_management("up_scale").getOutput(0)
			feat_count2 = arcpy.GetCount_management("uc_scale").getOutput(0)
			feat_count3 = arcpy.GetCount_management("us_scale").getOutput(0)
		...
		try:
			...
			with arcpy.da.SearchCursor("ugrid", ['OID@']) as scursor:
				for row in scursor:
					...
					arcpy.Integrate_management("up_scale 2;uc_scale 1;us_scale 3", "0.03 Meters")
					# Potentially update tolerance to 0.06m or just run it twice
			write("Freeing partition memory")
			arcpy.Delete_management("in_memory")
...
if dups:
	...
	write("\nCreating temporary output file: " + str(table_loc))
...
if repair:
	write("  - Repair All NULL Geometries\n")
if fcode:
	write("  - Populate F_Codes\n")
if defaults:
	write("  - Calculate Default Values\n")
if metrics:
	write("  - Calculate Metrics (AOO, ARA, LZN, and WID)\n")
if ufi:
