...
while hydro or hydro2:
	if hydro:
		...
		try:
			...
			if ap.MinimumBoundingGeometry_management(fc1, rectangle, "RECTANGLE_BY_AREA", "ALL", "", "").maxSeverity:
...
while trans or trans2:
	if trans:
		...
		try:
			...
			rectangle = "in_memory\\rectangle"
			write("Defining partition envelope")
			if ap.MinimumBoundingGeometry_management(fc2, rectangle, "RECTANGLE_BY_AREA", "ALL", "", "").maxSeverity:
				write("No curve features found. Nothing to Integrate. Moving to the next tool.")
				break
			with ap.da.SearchCursor(rectangle, ['SHAPE@']) as scursor:
				for row in scursor:
...
while util or util2:
	...
	if util2:
		...
		try:
			...
			rectangle = "in_memory\\rectangle"
			write("Defining partition envelope")
			if ap.MinimumBoundingGeometry_management(util_list[1], rectangle, "RECTANGLE_BY_AREA", "ALL", "", "").maxSeverity:
				write("No curve features found. Nothing to Integrate. Moving to the next tool.")
				break
			with ap.da.SearchCursor(rectangle, ['SHAPE@']) as scursor:
				for row in scursor:
