	...
	<xsl:template match="//toolbox" mode="gp">
		...
		<xsl:choose>
			...
			<xsl:when test="/metadata/idinfo/descript/abstract[(. != '') and not(contains(text(), 'REQUIRED')) and ($hasArcGISmetadata = 'False')]">
				<xsl:apply-templates select="/metadata/idinfo/descript/abstract" mode="gp" />
			</xsl:when>
			<xsl:otherwise>
				...
				<xsl:for-each select="parameters/param">
					<tr>
						...
						<td class="info" align="left">
						  <xsl:if test="not(*)"><p><span class="noContent">There is no explanation for this parameter.</span></p></xsl:if>
						  ...
						  <xsl:if test="not(dialogReference) and (pythonReference or commandReference)">
							<p><span style="font-weight: bold">Dialog Reference</span></p>
							<p><span class="noContent">There is no dialog reference for this parameter.</span></p>
						  </xsl:if>
						  <xsl:if test="pythonReference or commandReference">
							  ...
							  <xsl:for-each select="pythonReference">
								<xsl:if test="(position() = 1)">
									<span style="font-weight: bold">Python Reference</span><br />
								</xsl:if>
								<xsl:choose>
									<xsl:when test="(. = '') or not(node())"><p><span class="noContent">There is no python reference for this parameter.</span></p></xsl:when>
									<xsl:when test="para or bulletList or bullet_item or indent or subSection or bold or italics">
										<xsl:choose>
											<xsl:when test="not(name(*) != 'bullet_item')">
												<ul>
									...
									<xsl:when test="text()[(contains(.,'&lt;/')) or (contains(.,'/&gt;'))]">
										...
										<xsl:choose>
											...
											<xsl:otherwise>
												<p><span class="noContent">There is no python reference for this parameter.</span></p>
											</xsl:otherwise>
										</xsl:choose>
									</xsl:when>
									<xsl:when test=".//text()">
									...
									<xsl:otherwise><p><span class="noContent">There is no python reference for this parameter.</span></p></xsl:otherwise>
								</xsl:choose>
							  </xsl:for-each>
							  <xsl:if test="not(pythonReference) and (commandReference)">
								  <xsl:for-each select="commandReference">
									<xsl:if test="position() = 1">
									  <span style="font-weight: bold">Command Reference</span><br />
									</xsl:if>
									<xsl:choose>
										<xsl:when test="(. = '') or not(node())"><p><span class="noContent">There is no command reference for this parameter.</span></p></xsl:when>
										<xsl:when test="para or bulletList or bullet_item or indent or subSection or bold or italics">
											<xsl:choose>
												<xsl:when test="not(name(*) != 'bullet_item')">
													<ul>
										...
										<xsl:otherwise><p><span class="noContent">There is no command reference for this parameter.</span></p></xsl:otherwise>
									</xsl:choose>
								  </xsl:for-each>
							  </xsl:if>
						  </xsl:if>
						  <xsl:if test="not(pythonReference or commandReference) and dialogReference"><p><span class="noContent">There is no python reference for this parameter.</span></p></xsl:if>
						</td>
						<td class="info" align="left">
